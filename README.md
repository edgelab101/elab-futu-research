...
启动后 skill 会先确认五项参数（研究对象、时间范围、探量给预期、交付物、其他约束），已提供的不重复问，缺的一次性集中问完，确认后再开始抓取。**时间范围需要明确选择，没有静默默认**——skill 会列出选项让你选。
...
- 同时抓取”动态”和”专栏”；
- 启动时会询问时间范围（近半年 / 近 1 年 / 近 18 个月 / 全量历史），无静默默认，需明确选择；
...
抓取前 skill 会列出选项，请你明确选择时间范围——没有静默默认值：

| 选项 | CLI 参数 | 适合场景 |
|------|----------|----------|
| 近半年 | `--since YYYY-MM-DD`（半年前） | 快速了解近期风格 |
| 近 1 年 | `--since YYYY-MM-DD`（1 年前） | 覆盖完整市场周期 |
| 近 18 个月 | `--since YYYY-MM-DD`（18 个月前） | 跨越多个波动区间 |
| 全量历史 | 不加 `--since` | 完整档案，耗时较长 |

高产博主（数千帖以上）选全量历史时耗时较长（例：某博主逾万帖，全量抓取可能持续数小时，输出占数百 MB）。建议先按需选有界窗口，抓取结果支持断点续跑。

skill 会在正式抓取前先运行 `doctor` 探测帖量，给出”约 N 帖，全量约 X 小时/MB”的具体预期，供你参考再选。
